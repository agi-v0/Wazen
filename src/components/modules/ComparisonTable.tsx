		...
		altAppsLogos && (
			...
			>
				...
				<div className="w-full max-lg:overflow-x-scroll">
					<div className="mx-auto grid w-[672px] grid-cols-[repeat(calc(var(--col-count)+2),minmax(0,1fr))] lg:w-full">
						<div className="sticky start-0 col-span-2 mt-14 grid grid-flow-row grid-cols-1 grid-rows-[repeat(calc(var(--row-count)-1),minmax(0,1fr))] rounded-lg border-gray-200 bg-white *:border-t max-lg:w-48">
							{comparisonTable.rows.map((featureRow: any, index: any) => {
								return cellContent(featureRow.cells.slice(-1), index, locale)
							})}
						</div>
						<div className="col-span-5 grid w-[480px] grid-rows-[repeat(var(--row-count),minmax(0,1fr))] divide-y divide-gray-200 lg:w-full">
							<div
								className={`grid grid-flow-col grid-cols-[repeat(var(--col-count),minmax(0,1fr))] items-center justify-center gap-2 pb-4`}
							>
							...
							{comparisonTable.rows.map((featureRow: any, index: any) => {
								return (
									<div
										...
										key={index}
										className="grid grid-flow-col grid-cols-[repeat(var(--col-count),minmax(0,1fr))] gap-2 *:w-24 *:lg:w-full"
									>
										{featureRow.cells
